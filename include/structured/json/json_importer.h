...
//          `"""`            `"""`
// -----------------------------------------------------------------------------
// structured/json/json_importer.h
// -----------------------------------------------------------------------------
...
#ifndef koobika_hook_structured_json_jsonimporter_h
#define koobika_hook_structured_json_jsonimporter_h
...
struct DefaultNumericParser {
  ...
  JsonValue Parse(const std::string& str) const {
...
class JsonImporter {
 public:
  ...
  static JsonValue From(const char* content,
  ...
  // Parses the incoming string content to generate a json-value.
  template <typename NumericParser = DefaultNumericParser>
  static JsonValue From(const std::string& str,
                        const NumericParser& numeric_parser = NumericParser()) {
    return From(str.c_str(), numeric_parser);
  }
  // Parses the incoming string content to generate a json-value.
  template <typename NumericParser = DefaultNumericParser>
  static JsonValue From(std::ifstream&& stream,
                        const NumericParser& numeric_parser = NumericParser()) {
 ...
 private:
  ...
  static std::optional<JsonValue> From(const char* content,
    ...
    if (!valueType.has_value() && !in_collection) {
      ...
      if (type == JsonValueType::kNull) {
        ...
        json_value = numeric_parser.Parse(std::string(&content[beg], idx - beg));
        ...
        while (idx < length) {
          ...
          if (!escaped) {
            if (ch == kReverseSolidusCh_) {
              ...
              json_value = std::string(&content[beg], idx - beg - 1);
              break;
            }
          } else {
            switch (ch) {
  ...
  static std::optional<JsonValueType> checkValueType(
      const char* content, const std::size_t& length, const std::size_t& index) {
    if (index < length) {
