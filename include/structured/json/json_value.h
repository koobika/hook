...
class JsonObject;
class JsonArray;
static void copy(Container&, const JsonObject&);
static void copy(Container&, const JsonArray&);
static void copy(Container&, const JsonValue&);
// =============================================================================
// JsonValue                                                           ( class )
// -----------------------------------------------------------------------------
...
class JsonValue : public base::Serializable {
 ...
 protected:
  ...
  void assign(const JsonValue& value) { copy(*data_, value); }
  // Assigns the incoming array to the current data storage.
  void assign(const JsonArray& array) { copy(*data_, array); }
  // Assigns the incoming object to the current data storage.
  void assign(const JsonObject& object) { copy(*data_, object); }
  ...
  // Checks for the provided value to be equal than the stored one.
  bool isEqual(const JsonValue& in) const {
  ...
  // Checks for the provided value to be less than the stored one.
  bool isLessThan(const JsonValue& in) const {
  ...
  // Checks for the provided value to be greater than the stored one.
  bool isGreaterThan(const JsonValue& in) const {
    bool less_than = false;
    if (IsNumber() && in.IsNumber()) {
      less_than = this->operator double() > in.operator double();
    } else if (IsString() && in.IsString()) {
  ...
  // FRIENDs                                                       ( protected )
  //
  friend void copy(Container&, const JsonObject&);
  friend void copy(Container&, const JsonArray&);
  friend void copy(Container&, const JsonValue&);
};
// =============================================================================
// JsonObject                                                          ( class )
// -----------------------------------------------------------------------------
...
class JsonObject : public JsonValue {
 public:
  ...
  // CONSTRUCTORs/DESTRUCTORs                                         ( public )
  //
  JsonObject() { *data_ = Map(); }
  JsonObject(
      const std::initializer_list<std::pair<std::string, JsonValue>>& list)
      : JsonObject() {
    assign(list);
  }
  ...
  const JsonValue& operator[](const char* key) const { return Get(key); }
  JsonValue& operator[](const char* key) { return Get(key); }
  ...
  JsonValue& Add(const std::pair<std::string, JsonValue>& in) {
  ...
  // Returns the associated value for an existent key.
  JsonValue& Get(const std::string& key) { return std::get<Map>(*data_)[key]; }
  // Returns the associated value for an existent key.
...
class JsonArray : public JsonValue {
 public:
  ...
  // CONSTRUCTORs/DESTRUCTORs                                         ( public )
  //
  JsonArray() { *data_ = Vector(); }
  JsonArray(const JsonArray& in) : JsonArray() { *data_ = *in.data_; }
  JsonArray(JsonArray&&) noexcept = delete;
  JsonArray(const std::initializer_list<JsonValue>& list) : JsonArray() {
    assign(list);
  }
  ...
  // OPERATORs                                                        ( public )
  //
  JsonArray& operator=(const JsonArray& in) {
    *data_ = *in.data_;
    return *this;
  }
  JsonArray& operator=(JsonArray&&) = delete;
  JsonArray& operator=(const std::initializer_list<JsonValue>& list) {
    assign(list);
    return *this;
  }
  const JsonValue& operator[](const std::size_t& idx) const { return At(idx); }
  JsonValue& operator[](const std::size_t& idx) { return At(idx); }
  // ___________________________________________________________________________
  // METHODs                                                          ( public )
  //
  // Adds the specified value to the array.
  JsonValue& Add(const JsonValue& in) {
    std::get<Vector>(*data_).push_back(in);
    return *this;
  }
  // Erases the specified value (located by index) from the array.
  JsonValue& Erase(const std::size_t& position) {
  ...
  JsonValue& Erase(const std::size_t& first, const std::size_t& last) {
    auto& container = std::get<Vector>(*data_);
  ...
  const JsonValue& At(const std::size_t& idx) const {
  ...
  // Returns the associated value for an existent key.
  JsonValue& At(const std::size_t& idx) {
    auto& container = std::get<Vector>(*data_);
    if (idx >= container.size()) {
      // ((Error)) -> while querying for an out-of-bounds index!
      throw std::logic_error("Out of bounds!");
 ...
 protected:
  ...
  //
  // Assigns a list of elements.
  void assign(const std::initializer_list<JsonValue>& list) {
    for (auto const& element : list) {
      Add(element);
    }
  }
...
// =============================================================================
void copy(Container& out, const JsonObject& in) {
  copy(out, (const JsonValue&)in);
}
void copy(Container& out, const JsonArray& in) {
  copy(out, (const JsonValue&)in);
}
void copy(Container& out, const JsonValue& in) { out = *in.data_; }
}  // namespace koobika::hook::structured::json
